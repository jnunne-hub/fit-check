...
const App: React.FC = () => {
  ...
  const handleGarmentSelect = useCallback(async (garmentFile: File, garmentInfo: WardrobeItem) => {
    ...
    try {
      ...
      const currentPoseInstruction = POSE_INSTRUCTIONS[0]; // Always generate for the default pose first
      ...
      setCurrentOutfitIndex(prev => prev + 1);
      setCurrentPoseIndex(0); // Reset to default pose for new item
  ...
  const handleRemix = useCallback(async (prompt: string) => {
    if (!displayImageUrl || isLoading) return;

    setError(null);
    setIsLoading(true);
    setLoadingMessage(`Remixing with AI...`);
    
    try {
      const newImageUrl = await remixImage(displayImageUrl, prompt);
      ...
      setError(getFriendlyErrorMessage(err, 'Failed to apply edit'));
    } finally {
      setIsLoading(false);
      setLoadingMessage('');
    }
            ...
            <main className="flex-grow relative flex flex-col md:flex-row overflow-hidden">
              ...
              >
                  ...
                  <div className="p-4 md:p-6 pb-20 overflow-y-auto flex-grow flex flex-col gap-8">
                    ...
                    />
                    <AdjustmentPanel onRemix={handleRemix} isLoading={isLoading} />
                    <WardrobePanel
